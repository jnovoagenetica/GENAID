...
def _get_table_client(user_id):
    table = _get_aws_resource("dynamodb", user_id=user_id).Table(TABLE_NAME)
    try:
        table.load()  # Verifica que la tabla exista
        logger.debug(f"[DynamoDB] Using table: {TABLE_NAME}")
    except table.meta.client.exceptions.ResourceNotFoundException:
        logger.error(f"[DynamoDB] ERROR: Table '{TABLE_NAME}' not found! Check your .env.local.")
        raise
    return table

def _get_table_public_client():
    table = _get_aws_resource("dynamodb").Table(TABLE_NAME)
    try:
        table.load()  # Verifica que la tabla exista
        logger.debug(f"[DynamoDB] Using table (public): {TABLE_NAME}")
    except table.meta.client.exceptions.ResourceNotFoundException:
        logger.error(f"[DynamoDB] ERROR: Table '{TABLE_NAME}' not found! Check your .env.local.")
        raise
    return table
