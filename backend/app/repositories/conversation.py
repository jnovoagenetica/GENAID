...
def find_conversation_by_user_id(user_id: str) -> list[ConversationMeta]:
    ...
    query_params = {
        "KeyConditionExpression": Key("PK").eq(user_id) & Key("SK").begins_with(f"{user_id}#CONV#"),
    ...
    conversations = [
        ConversationMeta(
            ...
            model=json.loads(item.get("MessageMap", "{}")).get("system", {}).get("model", ""),
...
def find_conversation_by_id(user_id: str, conversation_id: str) -> ConversationModel:
    ...
    try:
        ...
        if item.get("IsLargeMessage", False):
            ...
            message_map = json.loads(s3_response["Body"].read().decode("utf-8"))
        else:
        ...
        conv = ConversationModel(
            ...
            message_map={
                k: MessageModel(
                    ...
                    parent=v.get("parent"),
                    create_time=float(v.get("create_time", 0)),
                    feedback=(
                        FeedbackModel(
                            thumbs_up=v["feedback"].get("thumbs_up"),
                            category=v["feedback"].get("category", ""),
                            comment=v["feedback"].get("comment", ""),
                        ) if v.get("feedback") else None
                    ),
                    used_chunks=[
                        ChunkModel(
...
def update_feedback(user_id: str, conversation_id: str, message_id: str, feedback: FeedbackModel):
